...
(async () => {
  ...
  logger.info(
    `Starting idm-comman v${version} with environment: ${config.appEnvName}`
  );
  ...
  await app.listen(config.appPort, config.appHost, () => {
    logger.info(
      `idm-command v${version} is running on ${config.appHost}:${config.appPort}`
    );
  });
