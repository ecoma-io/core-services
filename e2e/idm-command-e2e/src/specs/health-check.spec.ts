...
defineFeature(feature, (test) => {
  ...
  test('Kiểm tra liveness healthy trả về trạng thái healthy', ({
    ...
    given(
      'tiến trình dịch vụ `idm-command` đã được khởi động và đang chạy',
      () => {
        // beforeAll already started the environment; sanity check client
        expect(environment.idmCommandApiClient).toBeDefined();
      }
    );

    when('operator hoặc orchestrator gọi `GET /health/liveness`', async () => {
      response = await environment.idmCommandApiClient.get('/health/liveness');
    });
