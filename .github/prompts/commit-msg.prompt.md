...
4. **Does the change primarily affect a specific Nx project (80%+ of changes)?**
   ...
   - For apps: `resource-service`, `resource-migration`
   - For libs: `nestjs-typeorm`, `integration-environment`
   - For packages: `nestjs-exceptions`, `common`, `nestjs-filters`
   - **IMPORTANT:** For E2E test changes in `e2e/resource-e2e/`, use the **main project name** (`resource-service`), NOT the e2e project name
...
| -------------------------------- | ------------------- | ---------------------------------------------------- |
| `apps/resource-service/src/**`   | `resource-service`  | `feat(resource-service): add user pagination`        |
...
| `e2e/resource-e2e/**`            | `resource-service`  | `test(resource-service): add user creation e2e test` |
...
```
test(resource-service): add e2e tests for user endpoints
```
...
```
feat(resource-service): add user service
```
...
```
feat(resource-service,nestjs-typeorm): add pagination
```

✅ **Correct**: Choose the primary scope (where 80%+ changes are)

```
feat(resource-service): add cursor-based pagination
```
...
- ❌ `feat(resource-service): implement cursor-based pagination using TypeORM QueryBuilder`
- ✅ `feat(resource-service): add cursor-based pagination`

- ❌ `fix(resource-service): add null check in validation logic to prevent errors`
- ✅ `fix(resource-service): prevent null pointer in validation`
...
- If you updated 3 files but the core change is "add cursor-based pagination",
  ...
  - ✅ `feat(resource-service): add cursor-based pagination`
  - NOT: `feat(resource-service): update controller, service and add tests`
  - Avoid overly generic: `feat(resource-service): add pagination`
...
2. Component/Domain: resource-service | nestjs-typeorm | nestjs-exceptions | deps | ci | general
...
- `perf(resource-service): optimize user search with single join`
...
| -------------------------------------------------------------------------- | -------------------------------------------------------- |
| `feat(resource-service): add new pagination feature to user list endpoint` | `feat(resource-service): add user pagination`            |
| `fix(resource-service): fix the bug where null values cause errors`        | `fix(resource-service): handle null user profiles`       |
...
| `test(resource-service): add new integration test cases for user creation` | `test(resource-service): add user creation e2e tests`    |
...
```
feat(resource-service): add user pagination support
...
```
perf(resource-service): optimize user search query
