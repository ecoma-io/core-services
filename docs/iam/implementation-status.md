...
**Phiên bản báo cáo:** 1.8 (Phase 2.4 Complete - ServiceDefinition Vertical Slice) 🎉🎉🎉🎉🎉  
...
### 18/11/2025 - Phiên bản 1.8 (Phase 2.4 Complete - ServiceDefinition Vertical Slice) 🎉🎉🎉🎉🎉

| Mục                                  | Thay đổi                                       | Ghi chú                                                                     |
| ------------------------------------ | ---------------------------------------------- | --------------------------------------------------------------------------- |
| ServiceDefinitionProjector           | ✅ **HOÀN THÀNH**                              | Semver-based pruning (top 3 major versions with latest minor/patch)         |
| ServiceDefinitionsController         | ✅ **HOÀN THÀNH**                              | GET /service-definitions/:id endpoint + unit tests (4/4 PASS)               |
| ServiceDefinition Vertical Slice E2E | ✅ **PASS 100%**                               | service-definition-complete-flow.spec.ts - multi-version pruning (3/3 PASS) |
| RegisterServiceVersion Command       | ✅ **HOÀN THÀNH**                              | POST /commands/register-service-version endpoint + handler + DTO + repo     |
| Multi-Projector Support              | ✅ **5 Projectors Active**                     | Tenant + User + Role + Membership + ServiceDefinition running in parallel   |
| Semver Algorithm                     | ✅ **Advanced Logic**                          | Parse → group by major → keep latest → sort → take top 3                    |
| Permission Registry Foundation       | ✅ **Complete**                                | Services can register permission schemas dynamically                        |
| ADR-5 Implementation                 | ✅ **Top 3 Major Versions**                    | Automatic pruning when 4th major version registered                         |
| EventStoreDB Versioning Fix          | ✅ **Critical Fix**                            | Correct expectedVersion handling for multi-command aggregates               |
| Repository Stream Naming Fix         | ✅ **Fixed**                                   | ServiceDefinitionRepository uses correct stream name (serviceId)            |
| Phase 2.4 Status                     | ✅ **100% COMPLETE**                           | RegisterServiceVersion → ServiceDefinitionProjector → GetServiceDefinition  |
| Next Priority                        | **Remaining Command Endpoints (Phase 3)** (P1) | UpdateTenant, ChangePassword, AssignRole, RemoveRole, etc.                  |
| Version                              | Nâng lên 1.8                                   | Phase 2.4 ServiceDefinition vertical slice hoàn thành sau 7 iterations      |
...
### Điểm hoàn thành tổng thể: **~92%** 🎉🎉🎉🎉🎉

**Cập nhật quan trọng (v1.8 - Phase 2.4 Complete):**
...
- ✅ **Phase 2.4 ServiceDefinition vertical slice: 100% complete** (RegisterServiceVersion → ServiceDefinitionProjector → GetServiceDefinition)
- ✅ **ServiceDefinitionProjector implemented**: Semver-based pruning algorithm (top 3 major versions)
- ✅ **ServiceDefinitionsController implemented**: GET /service-definitions/:id + unit tests (4/4 PASS)
- ✅ **E2E tests: 46/47 pass** (command-e2e 42/43, projector-e2e 1/1, vertical-slice-e2e 15/15)
- ✅ **Multi-projector architecture**: 5 projectors running in parallel (Tenant + User + Role + Membership + ServiceDefinition)
- ✅ **5 complete vertical slices validated**: All core domain entities have end-to-end flows
- ✅ **Advanced semver algorithm**: Parse → group by major → keep latest → sort → take top 3
- ✅ **Dynamic permission registry**: Services can register schemas with version management
- ✅ **ADR-5 implemented**: Top 3 major versions retention policy with automatic pruning
- 🎯 **Next: Phase 3 - Remaining Command Endpoints** (UpdateTenant, ChangePassword, AssignRole, RemoveRole, etc.)
...
- ✅ 17/17 command handlers implemented (added RegisterServiceVersionHandler in v1.8)
- ✅ 5/17 handlers validated end-to-end via vertical slice E2E tests
...
- **MembershipProjector** ✅ **WORKING (v1.7)**: xử lý `UserAddedToTenant`, `RoleAssignedToUser`, `RoleRemovedFromUser`, cập nhật memberships_read_model
- **ServiceDefinitionProjector** ✅ **WORKING (v1.8)**: xử lý `ServiceVersionRegistered`, semver parsing, top 3 major versions pruning
...
- **iam-projector-worker app** ✅ **RUNNING**: 5 projectors active (Tenant + User + Role + Membership + ServiceDefinition)
...
- `libs/iam-query-interactor/src/handlers/get-membership.handler.ts` ✅ **NEW (v1.7)**
- `libs/iam-query-interactor/src/handlers/get-service-definition.handler.ts` ✅ **NEW (v1.8)**
...
- **GetMembershipHandler** ✅ **COMPLETE (v1.7)**: với MembershipReadRepository injection, unit tests (3/3 PASS), wired in MembershipsController
- **GetServiceDefinitionHandler** ✅ **COMPLETE (v1.8)**: với ServiceDefinitionReadRepository injection, unit tests (4/4 PASS), wired in ServiceDefinitionsController

❌ **Thiếu:**

- ❌ SearchUsers, SearchTenants, CheckPermission queries (P2)
- ❌ Query interfaces/DTOs cho advanced queries (search, filters, pagination)
...
### Phase 2.4: Service Definition Vertical Slice ✅ **100% COMPLETE**

**Status:** ✅ All components implemented and E2E validated (18/11/2025)

**Deliverables Achieved:**

- ✅ Commands: `RegisterServiceVersion` (handler + DTO + endpoint)
- ✅ Events: `ServiceVersionRegistered` (published to RabbitMQ)
- ✅ Projector: `ServiceDefinitionProjector` with semver-based pruning algorithm
- ✅ Queries: `GetServiceDefinition` (handler + controller)
- ✅ Endpoints: `POST /commands/register-service-version`, `GET /service-definitions/:id`
- ✅ E2E Tests: service-definition-complete-flow.spec.ts (3/3 PASS)
- ✅ Advanced Semver Algorithm: Parse → group by major → keep latest minor/patch → sort descending → take top 3
- ✅ ADR-5 Implementation: Top 3 major versions retention policy

**Key Implementations:**

- Complex semver parsing with major.minor.patch extraction
- Sophisticated pruning logic: groups versions by major, keeps latest of each major, sorts and takes top 3
- Multi-version registration support for single service (first aggregate supporting multiple commands)
- EventStoreDB expectedVersion fix: handles both new streams (-1) and existing streams (version + 1 for UnitOfWork)
- Repository stream naming fix: uses serviceId directly as stream name (consistent with other aggregates)
- PostgreSQL JSONB storage for versions array with permission trees
- Comprehensive E2E test: 6 version registrations across 4 majors, validates pruning to [4.0.0, 3.0.0, 2.1.0]

**Critical Bugs Fixed:**

1. Repository stream naming mismatch (ServiceDefinition-{id} → {id})
2. Handler expectedVersion calculation for multi-command aggregates (service.version + 1)
3. Aggregate rehydration version setting (uses lastEvent.position)
4. UnitOfWork expectedVersion transformation (subtracts 1 for existing streams)

**Actual Time:** 3-4 days (7 E2E iterations for debugging optimistic locking and stream naming)

**Business value delivered:**

- ✅ Dynamic permission registry fully operational
- ✅ Services can register permission schemas with version management
- ✅ Automatic pruning prevents unbounded growth while maintaining compatibility
- ✅ Foundation for multi-service permission federation (ADR-5)
- ✅ First aggregate supporting multiple commands validated
