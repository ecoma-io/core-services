...
- `init-databases.sh` — initialization script used on first start
...
- `POSTGRES_DATABASES` — database name(s) to create
...
- The Compose file uses the external network `dev-infras`; ensure that network exists (start `core-infras` first or create it manually).
