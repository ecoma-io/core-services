 ...
 *
 * @param {PublishExecutorOptions} options - Executor options.
...
export default async function publishExecutor(
  ...
  try {
    ...
    const interpolatedName = String(finalName).replace(
      /{(workspaceRoot|projectRoot|projectName|sourceRoot)}/g,
      (_, key: string) => {
        const val = replacements[key] ?? '';
        // sanitize paths for use in image names: trim slashes and replace separators with '-'
        return String(val)
          .replace(/^[/\\]+|[/\\]+$/g, '')
          .replace(/[/\\]+/g, '-');
      }
