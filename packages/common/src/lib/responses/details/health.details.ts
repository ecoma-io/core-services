...
/**
 * Runtime type guard for HealthDetails.
 * @param value - Value to check.
 * @returns {boolean} True if value appears to be a HealthDetails object.
 */
export function isHealthDetails(value: unknown): value is HealthDetails {
  if (value === null || typeof value !== 'object') return false;
  const obj = value as Record<string, unknown>;
  return Object.values(obj).every((v) =>
    Object.values(ServiceHealthStatus).includes(v as ServiceHealthStatus)
  );
}
