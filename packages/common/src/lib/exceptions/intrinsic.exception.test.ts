...
describe('intrinsicException', () => {
  ...
  it('handles absence of Error.captureStackTrace gracefully', () => {
    // Arrange: Temporarily remove captureStackTrace
    const orig = (Error as any).captureStackTrace;
    try {
      (Error as any).captureStackTrace = undefined;

      // Act: Create an exception when captureStackTrace is not available
      const ex = new IntrinsicException('no capture');

      // Assert: instance is created and message preserved
      expect(ex).toBeInstanceOf(IntrinsicException);
      expect(ex.message).toBe('no capture');
    } finally {
      (Error as any).captureStackTrace = orig;
    }
  });

  it('falls back to stable name when constructor name is unavailable', () => {
    // Arrange: Create an instance and temporarily remove its constructor
    const ex = new IntrinsicException('nm');
    const origConstructor = (ex as any).constructor;
    try {
      // Simulate missing constructor/name
      (ex as any).constructor = undefined;
      // Call protected initName via any cast to exercise fallback
      (ex as any).initName();

      // Assert: name is set to fallback
      expect(ex.name).toBe('IntrinsicException');
    } finally {
      (ex as any).constructor = origConstructor;
      // restore proper name
      (ex as any).initName();
    }
  });
