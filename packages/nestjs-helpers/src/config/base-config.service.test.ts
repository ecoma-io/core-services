...
jest.mock('class-transformer', () => ({
  ClassConstructor: jest.fn(),
}));

// Define a concrete subclass for testing since BaseConfigService is abstract
class TestConfigService extends BaseConfigService<{ key: string }> {
  constructor(validator: ClassConstructor<{ key: string }>) {
    super(validator);
  }

  public getEnvironments(): { key: string } {
...
describe('BaseConfigService', () => {
  ...
  let mockValidator: ClassConstructor<{ key: string }>;
  ...
  beforeEach(() => {
    ...
    >;
    mockValidator = {} as ClassConstructor<{ key: string }>;
